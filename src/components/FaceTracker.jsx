...
import { Camera, Eye, Smile, Activity, Zap } from 'lucide-react';
...
export default function FaceTracker({ isLive, onMetricsChange }) {
  ...
  return (
    <div className="glass-panel p-5 rounded-2xl border border-slate-200 relative overflow-hidden flex flex-col h-full bg-white">
      {/* HUD Header */}
      <div className="flex items-center justify-between mb-4">
        ...
        <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase ${
          metrics.eyeContact > 80 ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-amber-100 text-amber-800 border border-amber-200'
        }`}>
      ...
      {/* Video & Canvas Frame */}
      <div className="relative w-full aspect-video rounded-xl bg-slate-950 overflow-hidden border border-slate-800 mb-4 shadow-inner">
        <video
          ref={videoRef}
          autoPlay
          playsInline
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-auto">
        <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
          <div className="flex items-center justify-between text-slate-600 text-[11px] font-semibold mb-1">
            <span className="flex items-center gap-1"><Eye className="w-3 h-3 text-cyan-600" /> Eye Contact</span>
            <span className="text-cyan-700 font-mono font-extrabold">{metrics.eyeContact}%</span>
          </div>
          <div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden">
            <div className="bg-gradient-to-r from-cyan-600 to-indigo-600 h-full transition-all duration-300" style={{ width: `${metrics.eyeContact}%` }} />
          </div>
        </div>

        <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
          <div className="flex items-center justify-between text-slate-600 text-[11px] font-semibold mb-1">
            <span className="flex items-center gap-1"><Smile className="w-3 h-3 text-amber-600" /> Smile Score</span>
            <span className="text-amber-700 font-mono font-extrabold">{metrics.smileScore}%</span>
          </div>
          <div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden">
            <div className="bg-amber-500 h-full transition-all duration-300" style={{ width: `${metrics.smileScore}%` }} />
          </div>
        </div>

        <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
          <div className="flex items-center justify-between text-slate-600 text-[11px] font-semibold mb-1">
            <span className="flex items-center gap-1"><Activity className="w-3 h-3 text-emerald-600" /> Attention</span>
            <span className="text-emerald-700 font-mono font-extrabold">{metrics.attentionLevel}%</span>
          </div>
          <div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden">
            <div className="bg-emerald-500 h-full transition-all duration-300" style={{ width: `${metrics.attentionLevel}%` }} />
          </div>
        </div>

        <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
          <div className="flex items-center justify-between text-slate-600 text-[11px] font-semibold mb-1">
            <span className="flex items-center gap-1"><Zap className="w-3 h-3 text-purple-600" /> Nervousness</span>
            <span className="text-purple-700 font-mono font-extrabold">{metrics.nervousness}</span>
          </div>
          <div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden">
            <div className="bg-purple-500 h-full transition-all duration-300" style={{ width: metrics.nervousness === 'Low' ? '30%' : '75%' }} />
          </div>
        </div>
      </div>
    </div>
